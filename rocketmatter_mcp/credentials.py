#!/usr/bin/env python3
"""Pluggable credential storage for rocketmatter-mcp.

Secrets (API key, username, password) are stored in the operating system's
...
Storage order (read):  OS keyring  ->  process env  ->  ~/.rocketmatter-mcp/.env file
...
keyring with ``ROCKETMATTER_MCP_USE_KEYRING=0``.
...
_USE_KEYRING_FLAG = "ROCKETMATTER_MCP_USE_KEYRING"
# ---------------------------------------------------------------------------
...
def _keyring_enabled() -> bool:
    ...
    Opt out with ``ROCKETMATTER_MCP_USE_KEYRING=0`` (or false/no/off). A failing
